...
//   └──────────────────────┘
//
// **Мета стоит НАД ПОЛОСОЙ ТАБОВ, а не под шапкой** (решение Ивана 2026-07-30). Она
// давно перестала быть подписью к шапке: с 2026-07-26 её содержимое — счёт по табам
// в их порядке, то есть спойлер того, что ждёт внутри. Прижатая к табам, она читается
// как их оглавление («что меня ждёт во вкладках»); оторванная от них панелями
// `extra` — как продолжение шапки, и связь пропадала. Один слот на все формы: правится
// здесь, переезжает у всех сразу.
//
...
export function FormShell({ id, code, entity, fields, meta, extra, tabs, error, locked, ...cmd }:
  ...
  return (
    <div className={locked ? 'form-locked' : ''}>
      ...
      {/* Дополнение стека той же карточной раскладкой (§13.1) — панель бюджета проекта
          или интеграл контрагента: не список (табом быть не может) и не поле шапки. */}
      {extra}
