...
4. **Серверный `backend/.env`:** создать вручную ОДИН раз (в репозиторий не попадает).
   ...
   Плюс `ORG_COUNTERPARTY_ID` (волна 19, Ф4b) — **id нашей организации в справочнике
   контрагентов**: мы сами заведены обычным контрагентом, окружение лишь знает, кто из
   справочника «мы», и подписывает нашей стороной xlsx-бланк закупки. Значение зависит
   от базы, поэтому ставится ПОСЛЕ первой заливки данных; пустое не ломает выгрузку —
   бланк собирается без строки «Заказчик».
