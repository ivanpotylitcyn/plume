...
| `Procurement` | Закупка | **Планирование**: что и сколько решили купить у контрагента (экспертная оценка). Один поток общения = один xlsx-бланк. **Охват** (`projects`, M2M) — под какие проекты ведётся: он и задаёт область расчёта дефицита (волна 19, Ф13; раньше охват выражался *отсутствием* проекта — «командная высота»). Нарезается в Заказы. |
...
| `Receipt` | Поставка (УПД) | Приёмка по универсальному передаточному документу; рождает партии. Несёт контрагента-поставщика (`contractor`) и дату. (Прежнее имя термина — «Приход»; код/UI на «Поставку» с волны 14.) |
...
flowchart TD
  ...
  B --> ORD["Закупка (Procurement) под ОХВАТ проектов → xlsx-бланк<br/>нарезка по проектам в Заказы (Purchase), цены оффлайн"]
  ORD --> UPD["Поставка по УПД<br/>(Receipt → партии Lot)"]
  UPD --> W
...
erDiagram
  ...
  PROJECT ||--o{ ATTACHMENT : "ТЗ, договор"
  PROCUREMENT ||--o{ ATTACHMENT : "бланк запроса, КП"
  PURCHASE ||--o{ ATTACHMENT : "счёт, подтверждение"
  COUNTERPARTY ||--o{ ATTACHMENT : "карточка предприятия"
