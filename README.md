...
| `Procurement` | Закупка | **Планирование**: что и сколько решили купить у контрагента (экспертная оценка). Один поток общения = один xlsx-бланк, может охватывать несколько проектов. Нарезается в Заказы. |
...
| roll-up | Свод | Суммирование дефицита по изделиям через все проекты (командная высота). |
...
flowchart TD
  ...
  B --> ORD["Закупка (Procurement) → xlsx-бланк<br/>нарезка по проектам в Заказы (Purchase), цены оффлайн"]
...
- **Два дашборда — две высоты одного движка.** **Командный** (по всем проектам, ось
  «изделие») даёт плоский свод дефицита по номенклатуре и ведёт в `Procurement`
  (`Procurement` без `project` — маркер командной высоты, а не пробел): команда
  консолидирует «что закупить сейчас». **Проектный** (ось «проект») ведёт в `Purchase`
