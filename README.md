...
erDiagram
  ...
  PROJECT {
    ...
    string status "active/closed (жизненный цикл, не замок)"
    date started_at
...
- **Комплектация (`Kitting`) — инструмент ведения сборки лота, не атомарный акт.**
  ...
  «красный», пока акт в `draft`; при переводе в `posted` рождается `Lot` прибора
  (`+RECEIPT`-движение) — «проведён» в смысле «сделано всё, что можно». Брошенный акт
  не копит статус-надгробие — **отмена = удаление** (расфиксировать → удалить). Строки
  и движения `draft`-акта провизорны и правятся/удаляются, **пока акт не `posted`**;
  после — расфиксировать (`reopen`) или компенсирующим `RETURN`.
...
- **ДНК мутабельная + мягкий замок (agile «red → green»).** Команда идёт кругами
  ...
  сведено. **Единый мягкий замок складского документа — `StockDoc.status
  {draft ⇄ posted}`** (волна 13, Ф1): свернул разнородные `Receipt.approved`/
  `Transfer.posted`/`Kitting.{wip/closed}` в одну ось на всех 6 ордерах (Поставка/
  Комплектация/Инвентаризация/Требование/Передача/Списание). `posted` = edit-freeze
  (форма read-only); склад **не гейтится** статусом. `cancelled` снят — **отмена =
  удаление** (draft свободно; posted — сперва расфиксировать; `PROTECT` бережёт
  потраченные лоты). Проектный замок отдельно: `Project.status=closed`. Лот «заморожен»,
...
- **Закупки — два уровня: Закупка (`Procurement`) → Заказ (`Purchase`).**
  ...
  **слабая**: технически закупку после нарезки можно было бы удалить, не разрушая
  ...
  проектный документ со своим `status` (`draft/posted` — та же ось, что у ордеров и
  изделия), автором и датами (начало переговоров / подписание) — каждый кусок
  застревает на согласовании сам по себе; `Procurement.status` (`draft/posted`) —
  ...
  приходов, а не статус: замок (`draft/posted`) и покрытие (▲/●/✓) — две разные оси.
...
- **Бюджет и помощник-линза.** У `Project` есть `budget` (деньги на материалы =
  ...
  - **«Заказано» — отдельный (оранжевый) счётчик, не путать со «складом».** =
    ...
    `posted`** (реальное обязательство перед контрагентом); `draft` остаётся красным («ещё
    не утвердил»). Отсева по «отменённым» больше нет — отмена = удаление, а удалённого
