...
| `Counterparty` | Контрагент | Единая внешняя сторона документооборота с ролями (`is_supplier`/`is_customer`): поставщик закупки/заказа/поставки и/или заказчик передачи. Свернул прежний `Supplier` (волна 13, Ф2f+). **Наша организация — тоже обычный контрагент** (волна 19, Ф4b): правится той же формой, несёт те же реквизиты и карточку предприятия; окружение знает лишь её id (`ORG_COUNTERPARTY_ID`) — этим xlsx-бланк закупки подписывается нашей стороной, а название организации не попадает в репозиторий. Отсюда же бесплатно выходит закупка «у себя». |
