...
- **Закупки — два уровня: Закупка (`Procurement`) → Заказ (`Purchase`).** **Закупка =
  ...
  проектный документ со своим `status` (`draft/sent/partial/received/cancelled`),
  автором и датами (начало переговоров / подписание) — каждый кусок застревает на
  согласовании сам по себе; `Procurement.status` (`draft/sent/cancelled`) — состояние
  всего потока. Заказ «разрешается» в приходы: `PurchaseLine` закрывается одной или
...
- **Бюджет и помощник-линза.** У `Project` есть `budget` (деньги на материалы =
  ...
  - **«Заказано» — отдельный (оранжевый) счётчик, не путать со «складом».** =
    ...
    `sent`** (реальное обязательство перед контрагентом); `draft` остаётся красным («ещё
    не отправил»), `cancelled` не в счёт. **Поступившее = приход (`+RECEIPT`), не текущий
