...
erDiagram
  ...
  STOCKMOVEMENT {
    ...
    int location_id FK
    string type "RECEIPT/ISSUE/RETURN (выводится из origin+знак)"
...
- **Комплектация (`Kitting`) — инструмент ведения сборки лота, не атомарный акт.**
  ...
  компонента, бизнес-дата = `KittingLine.date`) в реестр движений — так комплектацию
  фиксируем по ходу, а не вспоминаем в конце, когда приборы уже уехали. Лот прибора
  «красный», пока акт расфиксирован; на фиксации рождается `Lot` прибора
  (`+RECEIPT`-движение) — «сделано всё, что можно». Брошенный акт
  не копит статус-надгробие — **отмена = удаление** (расфиксировать → удалить). Строки
  и движения расфиксированного акта провизорны и правятся/удаляются **до фиксации**;
  после — расфиксировать (`unlock`) или компенсирующим `RETURN`.
...
документа; периодическая
  ...
  **только по `Lot`** (`item` и `project` выводятся из партии); остаток = сумма
  ...
  `RETURN`**, выводится из вида origin-документа и знака (никаких `ADJUSTMENT` /
  межскладского `TRANSFER` — под ними нет документа-источника). **Движение не существует
...
- **Поток «Заказ → УПД» — в одну кнопку** (волна 19, Ф6). В 98% случаев накладная
  ...
  (`qty − уже поступившее`) — поставщик приезжает из `Procurement.contractor` (закупка
  и есть «один поток общения»), заказ-источник проставляется сам. **Поставка частями**
...
- **Бюджет и помощник-линза.** У `Project` есть `budget` (деньги на материалы =
  ...
  - **Частичное покрытие — не бинарно.** Строка показывает **тройной разбор**
    ...
    пайке (`KittingLine`, кто первый) — привязка раньше была бы «резервом».
