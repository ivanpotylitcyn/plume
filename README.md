...
- Закупки и приход по УПД, склад с учётом партий и мест хранения (реестр движений).
...
документа; периодическая
  ...
  **только по `Lot`** (`item` и `project` выводятся из партии); остаток = сумма
  ...
  без документа** (`source_type` + `source_id` обязательны): приход (УПД), акт, передача.
- **ДНК мутабельная + мягкий замок (agile «red → green»).** Команда идёт кругами
  ...
  `Transfer.posted`/`Kitting.{wip/closed}` в одну ось на всех 6 ордерах (Приход/
...
- **Проект — свойство лота (`Lot.project`), не движения.** Лот живёт в одном
  ...
  по служебному `kind`).
- **Один УПД ↔ один проект.** Приход (`Receipt`) рождает партии только в своём
