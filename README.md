...
- Закупка под заданное количество изделий: разузлование состава (BOM), расчёт
  дефицита (надо − склад − заказано) и экспорт `order.xlsx` для поставщиков.
...
flowchart TD
  ...
  B --> ORD["Закупка (Procurement) → order.xlsx<br/>нарезка по проектам в Заказы (Purchase), цены оффлайн"]
...
- **Закупки — два уровня: Закупка (`Procurement`) → Заказ (`Purchase`).**
  ...
  **слабая**: технически закупку после нарезки можно было бы удалить, не разрушая
  заказы. Мы их всё же храним — как историю переговоров и источник `order.xlsx`, —
  ...
  контрагентом (одна сводная закупка = один `order.xlsx`), может охватывать несколько
