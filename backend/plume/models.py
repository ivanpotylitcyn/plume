...
class Procurement(models.Model):
    ...
    # явный: итог собирается ТОЛЬКО из отмеченных, отметить все = прежний «командный
    # свод», отметить один = прозрачная закупка под проект. M2M задаёт **область**
    # расчёта, а не хранит числа — дефицит остаётся проекцией, не таблицей.
